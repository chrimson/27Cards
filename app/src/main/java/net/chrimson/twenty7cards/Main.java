...
public class Main {
  public static void main(String[] args) {
    ...
    // For 3 rounds, place cards in respective columns, each time selecting which columns
    for (int round = 0; round < 3; round ++) {
      System.out.print("\n");
      int l = round % 3;
      int m = (round + 1) % 3;
      int n = (round + 2) % 3;
      for (colPlan[l] = 0; colPlan[l] < 3; colPlan[l]++) {
        System.out.printf("%2d: ", colPlan[l]);
        for (colPlan[m] = 0; colPlan[m] < 3; colPlan[m]++) {
          for (colPlan[n] = 0; colPlan[n] < 3; colPlan[n]++) {
